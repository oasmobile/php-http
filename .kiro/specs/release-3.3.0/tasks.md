...
- [~] 2. Routing 模块：行为审计 + 场景测试 + 修复
  ...
  - [ ] 2.2 执行 Routing 模块行为审计
    - 研读 Silex Routing 官方文档 + GitHub 源码
    - 枚举 Silex Routing API_Surface（YAML route loading、route parameter replacement、route caching、URL matching、URL generation、route collection manipulation、namespace prefix、inheritable route loading）
    - 逐项对比 `CacheableRouterProvider` 实现，分类为 covered / missing-non-breaking / missing-breaking / intentionally-removed
...
- [ ] 3. Middleware 模块：行为审计 + 场景测试 + 修复
  ...
  - [ ] 3.2 执行 Middleware 模块行为审计
    - 研读 Silex Middleware 官方文档 + GitHub 源码（`Application::before/after`）
    - 枚举 Silex Middleware API_Surface（`before()` / `after()` registration、priority ordering、`EARLY_EVENT` / `LATE_EVENT`、master-request-only filtering、short-circuit behavior、`MiddlewareInterface` / `AbstractMiddleware` contracts）
    - 逐项对比当前 Middleware_Chain 实现，分类为 covered / missing-non-breaking / missing-breaking / intentionally-removed
...
- [ ] 4. CORS 模块：行为审计 + 场景测试 + 修复
  ...
  - [ ] 4.2 执行 CORS 模块行为审计
    - 研读 Silex CORS 处理相关文档 + GitHub 源码
    - 枚举 Silex CORS API_Surface（preflight detection、`Access-Control-*` header processing、strategy matching、origin validation、credentials support、max-age、`MethodNotAllowed` exception handling、Security interaction）
    - 逐项对比 `CrossOriginResourceSharingProvider` 实现，分类为 covered / missing-non-breaking / missing-breaking / intentionally-removed
...
- [ ] 5. Error Handling 模块：行为审计 + 场景测试 + 修复
  ...
  - [ ] 5.2 执行 Error Handling 模块行为审计
    - 研读 Silex Error Handling 官方文档 + GitHub 源码（`Application::error()`）
    - 枚举 Silex Error Handling API_Surface（`error()` callback registration、error handler priority、exception-to-response conversion、`ExceptionListenerWrapper` behavior、chain short-circuit、chain passthrough、HTTP exception status code preservation、`FallbackViewHandler` error rendering）
    - 逐项对比当前 Error_Handler_Chain 实现，分类为 covered / missing-non-breaking / missing-breaking / intentionally-removed
...
- [ ] 6. Twig 模块：行为审计 + 场景测试 + 修复
  ...
  - [ ] 6.2 执行 Twig 模块行为审计
    - 研读 Silex Twig 集成文档 + GitHub 源码
    - 枚举 Silex Twig API_Surface（`TwigEnvironment` initialization、template path configuration、strict variables mode、auto-reload behavior、`getTwig()` access、Twig absence handling、Twig 3.x class name migration）
    - 逐项对比 `SimpleTwigServiceProvider` 实现，分类为 covered / missing-non-breaking / missing-breaking / intentionally-removed
...
- [ ] 7. Cookie 模块：行为审计 + 场景测试 + 修复
  ...
  - [ ] 7.2 执行 Cookie 模块行为审计
    - 研读 Silex Cookie 处理相关文档 + GitHub 源码
    - 枚举 Silex Cookie API_Surface（`ResponseCookieContainer` injection、cookie writing to response headers on `KernelEvents::RESPONSE`、cookie container lifecycle per request）
    - 逐项对比 `SimpleCookieProvider` 实现，分类为 covered / missing-non-breaking / missing-breaking / intentionally-removed
...
- [ ] 8. MicroKernel 聚合层：汇总审计 + 场景测试
  ...
  - [ ] 8.2 执行 MicroKernel 聚合层汇总审计
    - 枚举 MicroKernel public API methods（`run()`, `handle()`, `isGranted()`, `getToken()`, `getUser()`, `getTwig()`, `getParameter()`, `addExtraParameters()`, `addControllerInjectedArg()`, `addMiddleware()`, `addRoute()`, `addRoutes()`, `getCacheDirectories()`），验证各方法行为与 Silex-era `SilexKernel` 等价
