    ...
    public function match($pathinfo)
    {
        ...
        if (preg_match('#^(?P<game>[^\\.]++)\\.test\\.oasgames\\.com$#si', $host, $hostMatches)) {
            ...
            if (preg_match('#^/(?P<lang>[^/]++)/$#s', $pathinfo, $matches)) {
                return $this->mergeDefaults(array_replace($hostMatches, $matches, array('_route' => 'home')), array (  '_controller' => 'Oasis\\Mlib\\Http\\Ut\\ZxcController::home',));
            }

            // play.server
            if (preg_match('#^/(?P<lang>[^/]++)/play/server/$#s', $pathinfo, $matches)) {
                return $this->mergeDefaults(array_replace($hostMatches, $matches, array('_route' => 'play.server')), array (  '_controller' => 'Oasis\\Mlib\\Http\\Ut\\ZxcController::playServer',));
            }
