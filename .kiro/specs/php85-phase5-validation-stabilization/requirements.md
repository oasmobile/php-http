...
Phase 0–4 已完成内部依赖升级（PHPUnit 13.x、oasis/utils ^2.0、oasis/logging ^2.0）、框架替换（Silex → Symfony MicroKernel + Symfony 7.x）、Twig 3.x / Guzzle 7.x 升级、Security 组件 authenticator 系统重写，以及 PHP 语言层面 breaking changes 修复和代码现代化。项目代码已在 PHP 8.5 下可编译运行，`--testsuite all` 全量通过且无 deprecation notice。
...
- C-3: PBT 使用 Eris 1.x
- C-4: spec 级 DoD：tasks 全部完成 + `--testsuite all` 全量通过 + PHPStan 通过 + 无 deprecation notice
...
- **All_Suite**: `phpunit.xml` 中定义的 `all` 测试套件（`--testsuite all`），包含 `ut/` 目录下所有测试
...
5. WHEN `phpunit --testsuite all` is executed after the upgrade, THE test runner SHALL report all tests passing
...
3. WHEN `phpunit --testsuite all` is executed after the upgrade, THE test runner SHALL report all tests passing
...
**User Story:** 作为库维护者，我希望 Phase 5 完成后 `--testsuite all` 全量通过，以确认所有升级和修复工作成功。
...
- [语体] R5 AC1/AC2、R6 AC1 中 `phpunit --testsuite all` / `phpunit --testsuite pbt` 改为引用 Glossary 术语 All_Suite / PBT_Suite，消除 All_Suite 和 PBT_Suite 的孤立术语问题
