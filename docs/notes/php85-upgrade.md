...
1. **Test Baseline — 测试覆盖补全**（PRP-001）：在当前 PHP + PHPUnit 环境下，为所有缺少测试的模块补充全面的单元测试和集成测试，建立行为基线
2. **Phase 0 — 前置准备**（PRP-002）：升级 `oasis/logging`、`oasis/utils` 到 PHP 8.5 兼容版本；升级 PHPUnit 到 13.x 并适配测试
3. **Phase 1 — 框架替换**（PRP-003）：用 Symfony MicroKernel（或等价方案）替换 Silex，同时升级全部 Symfony 组件到 7.x
4. **Phase 2 — 模板与 HTTP 客户端**（PRP-004）：升级 Twig 到 3.x，Guzzle 到 7.x
5. **Phase 3 — 安全组件重构**（PRP-005）：适配 Symfony Security 7.x 的新 authenticator 系统
6. **Phase 4 — PHP 语言适配**（PRP-006）：修复隐式 nullable 参数、动态属性、类型严格化等语言层面问题
7. **Phase 5 — 验证与稳定**（PRP-007）：全量测试、静态分析（PHPStan/Psalm level 提升）、CI 矩阵覆盖 PHP 8.4 + 8.5
8. **Migration Guide**（PRP-008）：为下游消费者提供迁移文档和工具

每个 Phase 作为独立的 proposal 立项，避免单次变更过大。测试补全（PRP-001）从原 Phase 0 中拆出，确保在依赖升级前先用当前环境建立行为基线。

### 分支策略

- PRP-001 在独立的 `feature/php85-test-baseline` 上完成，merge 回 develop
- PRP-002 至 PRP-007（Phase 0–5）共享同一个长生命周期 feature branch `feature/php85-upgrade`，从 develop 拉出
- 各 Phase 在该 branch 上按依赖顺序逐个推进，每个 PRP 独立开 spec
- **branch 级 DoD**：全量 PHPUnit 通过（`phpunit`）+ PRP-007 scope 完成后，才 merge 回 develop
- **spec 级 DoD**：该 spec 的 tasks 全部完成 + 该 Phase 预期通过的 suite 实际通过（各 PRP 的 Branch Strategy 中列出具体 suite）
- 理由：Phase 0 升级依赖后，工程在 PHP 8.x 下会有大量兼容性问题（Silex、Symfony 4.x、Twig 1.x 均不兼容），直到 Phase 4–5 全部完成才能恢复全绿状态；中间状态不应进入 develop
