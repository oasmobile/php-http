...
- [x] 7. Core 模块（MicroKernel、Middleware、Configuration、Cookie、EventSubscribers、Exceptions）
  ...
  - [x] 7.12 确认无需修改的文件
    ...
    - _Requirements: 8.5_
  - [x] 7.13 Checkpoint: 运行 `phpunit` 确认 `src/` 全部修改后测试通过，commit
...
- [x] 8. 测试辅助类和测试文件修改（`ut/`）
  ...
  - [x] 8.6 修改 `ut/AwsTests/ElbTrustedProxyTest.php`
    ...
    - _Requirements: 3.1_
  - [x] 8.7 Checkpoint: 运行 `phpunit` 确认全部通过，无 deprecation notice，commit
...
- [x] 9. 元数据和文档更新（Design CR Q4=C）
  ...
  - [x] 9.3 Checkpoint: 运行 `phpunit` 确认全量通过，无 deprecation notice，commit
...
- [x] 10. 手工测试
  ...
  - [x] 10.3 验证零 deprecation notice
    - 运行 `phpunit`，确认输出中无 deprecation notice（来自 `src/` 和 `ut/` 的代码）
...
- Task 2–6 各自运行对应 suite + pbt suite 验证；Task 7 运行 `phpunit` 作为 `src/` 全量验证；Task 8 运行 `phpunit` 作为最终全量验证
