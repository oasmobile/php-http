...
Phase 3 完成后，`security`、`integration`、`pbt` 等 suite 已通过。但由于 PHP 语言层面 breaking changes 尚未修复，全量测试（`phpunit`）可能存在因 deprecation notice 或隐式行为变更导致的零星失败。
...
- 预期通过的 suite：`phpunit` 全量通过，无 deprecation notice
