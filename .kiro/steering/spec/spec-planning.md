...
- **架构参考**：如果 `graphify-out/GRAPH_REPORT.md` 存在，在需要理解项目结构、模块依赖、受影响范围时，优先使用 graphify 子命令（`/graphify query`、`/graphify explain`、`/graphify path` 等）进行结构化查询，而不是直接读取源文件。具体用法参见 `graphify.md` steering
