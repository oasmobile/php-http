...
3. **全量验证**（R5、R6、R7）：`--testsuite all` 全量通过 + PHPStan 零错误 + 零 deprecation notice，作为 branch 级 DoD 最终验证点
...
- **测试失败**：升级后运行 `--testsuite all`，根据失败信息定位 API 不兼容点。
...
1. `vendor/bin/phpunit --testsuite all` — 全量测试通过
