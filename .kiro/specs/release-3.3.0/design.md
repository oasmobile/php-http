...
| CR 问题 | 决策 | 设计体现 |
|---------|------|----------|
| Q1=A | 每个模块一个独立测试类，放在 `tests/` 对应子目录 | 见 [测试目录结构](#测试目录结构) |
...
**文件位置**：`tests/Helpers/ScenarioTestCase.php`
...
根据 CR Q1=A 决策，每个模块一个独立测试类，放在 `tests/` 对应子目录中：
...
- 回归测试文件命名：`{Module}FixRegressionTest.php`，放在对应模块的 `tests/` 子目录中
...
#### Security 场景测试（R2）— `tests/Security/SecurityScenarioTest.php`
...
#### Routing 场景测试（R4）— `tests/Routing/RoutingScenarioTest.php`
...
#### Middleware 场景测试（R6）— `tests/Middlewares/MiddlewareScenarioTest.php`
...
#### CORS 场景测试（R8）— `tests/Cors/CorsScenarioTest.php`
...
#### Error Handling 场景测试（R10）— `tests/ErrorHandlers/ErrorHandlerScenarioTest.php`
...
#### Twig 场景测试（R12）— `tests/Twig/TwigScenarioTest.php`
...
#### Cookie 场景测试（R14）— `tests/Cookie/CookieScenarioTest.php`
...
#### MicroKernel 聚合层场景测试（R16）— `tests/Integration/MicroKernelAggregationScenarioTest.php`
...
| 资源类型 | 位置 | 用途 |
|----------|------|------|
| YAML routes 文件 | `tests/{Module}/scenario.routes.yml` | 场景测试专用路由定义 |
| Twig 模板文件 | `tests/Twig/templates/scenario/` | Twig 场景测试专用模板 |
| Test Controller | `tests/Helpers/Controllers/ScenarioController.php` | 场景测试专用 controller（如需要） |
| Test Middleware | `tests/Helpers/Middlewares/` | 场景测试专用 middleware 实现 |
| Test Authenticator | `tests/Helpers/Security/` | 场景测试专用 authenticator 实现 |
...
| 变更 | 说明 |
|------|------|
| 新增 `tests/Helpers/ScenarioTestCase.php` | 场景测试基类，所有模块场景测试继承 |
| 新增 8 个 `*ScenarioTest.php` 文件 | 分布在 `tests/` 各模块子目录 |
...
- 现有 Cross-Community Integration Tests（community 19）与 design 中 `tests/Integration/` 的定位一致
...
**Q: 为什么 MicroKernel 聚合层场景测试放在 `tests/Integration/` 而不是单独目录？**
A: MicroKernel 聚合层测试本质上是跨模块集成测试，放在 `tests/Integration/` 与现有的 `SilexKernelCrossCommunityIntegrationTest` 等文件保持一致。
