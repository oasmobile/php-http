...
验证策略：全量测试（phpunit 510 tests）+ 静态分析（phpstan level 8）+ Migration Guide 端到端手工验证 + Check Script 端到端手工验证（Goal CR Q1=B）。
...
release/3.0 分支
  ├── Phase 1: 全量测试确认 + 静态分析
  │     ├── phpunit 全量测试（510 tests, 16642 assertions）
...
预期结果：510 tests, 16642 assertions, 0 failures, 0 errors, 0 deprecation notices。
...
- 全量测试：510 tests, 16642 assertions（全部通过）
- PHPStan level 8：零错误
...
| Phase 5（PRP-007） | 全量验证 + PHPStan | 510 tests, 16642 assertions |
...
- Phase 5（PRP-007）已完成全量验证（510 tests + PHPStan level 8 + 零 deprecation）
...
- **全量测试**：`php vendor/bin/phpunit`，预期 510 tests, 16642 assertions, 0 failures, 0 errors
