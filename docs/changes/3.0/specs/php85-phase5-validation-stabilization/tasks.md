...
- [x] 1. oasis/utils ^3.0 升级（R1）
  - [x] 1.1 执行 `composer require oasis/utils:^3.0`，记录编译错误和测试失败
    - 运行 `composer require oasis/utils:^3.0`
    - 运行 `phpunit` 收集失败信息
    - 记录所有 API 变更点（方法签名、类型常量、异常类等）
    - _Ref: R1 AC1, R1 AC6_
  - [x] 1.2 适配 `src/` 中所有 oasis/utils ^3.0 API 变更
    ...
    - _Ref: R1 AC2, R1 AC4_
  - [x] 1.3 适配 `ut/` 中所有 oasis/utils ^3.0 API 变更
    ...
    - _Ref: R1 AC2, R1 AC3_
  - [x] 1.4 Checkpoint: 运行 `phpunit` 确认全量通过，commit
    - _Ref: R1 AC5_

- [x] 2. oasis/logging ^3.0 升级（R2）
  - [x] 2.1 执行 `composer require oasis/logging:^3.0`，记录编译错误和测试失败
    - 运行 `composer require oasis/logging:^3.0`
    - 运行 `phpunit` 收集失败信息
    - _Ref: R2 AC1, R2 AC4_
  - [x] 2.2 适配 oasis/logging ^3.0 API 变更
    - 影响范围极小，仅 `ut/bootstrap.php` 中 `LocalFileHandler` 的构造和 `install()` 调用
    - 如有 API 变更，适配 `(new LocalFileHandler('/tmp'))->install()` 调用
    - _Ref: R2 AC2_
  - [x] 2.3 Checkpoint: 运行 `phpunit` 确认全量通过，commit
    - _Ref: R2 AC3_

- [x] 3. PHPStan 引入与错误修复（R3、R4）
  - [x] 3.1 安装 PHPStan 并创建配置文件
  ...
  - [x] 3.3 Checkpoint: 运行 `vendor/bin/phpstan analyse` 确认零错误，运行 `phpunit` 确认全量通过，commit
    - _Ref: R3 AC6, R4 AC1_

- [x] 4. 文档统一更新（R8、R9、R10、R11）
  - [x] 4.1 更新 `PROJECT.md`
  ...
  - [x] 4.5 Checkpoint: 通过 grep 搜索 "Silex"、"Pimple"、"SilexKernel"、"Symfony 4" 确认文档无遗漏，commit

- [x] 5. PBT 验证（R12）
  - [x] 5.1 编写 Property 1 测试：ArrayDataProvider round-trip invariant
  ...
  - [x] 5.2 编写 Property 2 测试：ArrayDataProvider non-existent key error condition
...
- [x] 7. 最终验证（R5、R6、R7）
  ...
  - [x] 7.2 零 Deprecation Notice 验证
    - 运行 `phpunit`，确认输出中无来自 `src/` 和 `ut/` 的 deprecation notice
...
- [x] 8. 手工测试
  ...
  - [x] 8.5 Checkpoint: 手工测试全部通过，commit

- [x] 9. Code Review
  - [x] 9.1 委托给 code-reviewer agent 执行
  - [x] 9.2 Checkpoint: Code review 通过，处理所有 review 意见，commit
